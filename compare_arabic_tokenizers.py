...
def main():
    print("Running Arabic Corpus Tokenizer Benchmarking and Training Comparison...")

    arabic_path = "data/arabic_input.txt"
    if not os.path.exists(arabic_path):
        sample_text = "كان يا مكان في قديم الزمان قرية جميلة في الجبال. العلم نور والجهل ظلام."
    else:
        with open(arabic_path, "r", encoding="utf-8") as f:
            sample_text = f.read()

    short_sample = "كان يا مكان في قديم الزمان، قرية جميلة يعيش فيها أناس طيبون يحبون العلم والمعرفة."

    tokenizers = {}

    print("Fitting Arabic Character Tokenizer...")
    char_tok = ArabicCharacterTokenizer(sample_text)
    
    print("Fitting BPE Tokenizer on Arabic Corpus...")
    bpe_tok = BPETokenizer(vocab_size=128)
    bpe_tok.fit(sample_text)
    tokenizers["Standard BPE"] = bpe_tok

    print("Fitting WordPiece Tokenizer on Arabic Corpus...")
    wp_tok = WordPieceTokenizer(vocab_size=128)
    wp_tok.fit(sample_text)
    tokenizers["WordPiece"] = wp_tok

    print("Fitting SentencePiece Tokenizer on Arabic Corpus...")
    sp_tok = SentencePieceTokenizer(vocab_size=128, model_prefix="spm_arabic")
    sp_tok.fit(sample_text, input_file=arabic_path)
    tokenizers["SentencePiece"] = sp_tok

    print("Fitting Byte-BPE Tokenizer on Arabic Corpus...")
    byte_bpe_tok = ByteBPETokenizer(vocab_size=128)
    byte_bpe_tok.fit(sample_text)
    tokenizers["Byte-Level BPE"] = byte_bpe_tok

    print("Fitting Regex-BPE Tokenizer on Arabic Corpus...")
    arabic_pattern = r"""[\u0600-\u06FF]+|\d+|[^\s\w\d]+|\s+"""
    regex_tok = RegexBPETokenizer(vocab_size=128, pattern=arabic_pattern)
    regex_tok.fit(sample_text)
    tokenizers["Regex BPE"] = regex_tok

    print("Fitting GPT Tokenizer on Arabic Corpus...")
    gpt_tok = GPTTokenizer(vocab_size=128)
    gpt_tok.tokenizer.pattern = arabic_pattern
    gpt_tok.tokenizer.compiled_pattern = re.compile(arabic_pattern) if hasattr(gpt_tok.tokenizer, "compiled_pattern") else None
    ...
    for name, tok in tokenizers.items():
        ...
        def make_get_batch(t):
            def batch_fn(split):
                if hasattr(t, "get_batch"):
                    return t.get_batch(split, data_path=arabic_path)
        ...
        val_l, p_pl, spd, _, g_ids = train_and_eval_model(v_size, make_get_batch(tok), max_iters=50)
    ...
    ax1.set_title("Arabic Compression Ratio (Bytes / Token)", color="white", fontweight="bold")
    ...
    ax2.set_title("Arabic GPT Validation Cross-Entropy Loss", color="white", fontweight="bold")
    ...
    ax3.set_title("Arabic Perplexity (Lower is Better)", color="white", fontweight="bold")
    ...
    ax4.set_title("Arabic Generation Speed (Tokens / Sec)", color="white", fontweight="bold")
    ...
    plt.suptitle("Arabic Corpus Tokenizer Benchmarks & Model Performance", color="white", fontweight="bold", fontsize=14)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.savefig("assets/arabic_tokenizer_comparison.png", dpi=300, bbox_inches="tight")
    plt.close()
    print("Saved Arabic tokenizer comparison plot to assets/arabic_tokenizer_comparison.png")

    print("\n--- Summary of Arabic Tokenizer Benchmarking Results ---")
    for r in results:
