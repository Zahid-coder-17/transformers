...
from gpt import GPT
from tokenization.character import vocab_size as char_vocab_size, encode as char_encode, decode as char_decode, get_batch as char_get_batch
...
def main():
    print("Running Tokenizer Benchmarking and Training Comparison...")

    with open("data/input.txt", "r", encoding="utf-8") as f:
        sample_text = f.read(5000)

    short_sample = "Once upon a time, in a bright little house, there lived a curious girl named Lily. She loved to explore!"
    ...
    char_ids = char_encode(short_sample)
    char_comp = raw_bytes / len(char_ids)
    val_loss, ppl, speed, _, gen_ids = train_and_eval_model(char_vocab_size, char_get_batch, max_iters=50)
    gen_text = char_decode(gen_ids)
    ...
    ax1.set_title("Compression Ratio (Bytes / Token)", color="white", fontweight="bold")
    ...
    ax2.set_title("Validation Cross-Entropy Loss", color="white", fontweight="bold")
    ...
    ax3.set_title("Perplexity (Lower is Better)", color="white", fontweight="bold")
    ...
    ax4.set_title("Inference Speed (Tokens / Sec)", color="white", fontweight="bold")
    ...
    plt.suptitle("Tokenizer Comparison: Character vs BPE vs WordPiece vs SentencePiece vs Byte-BPE vs Regex BPE vs GPT Tokenizer", color="white", fontweight="bold", fontsize=13)
    plt.tight_layout(rect=[0, 0, 1, 0.95])
    plt.savefig("assets/tokenizer_comparison.png", dpi=300, bbox_inches="tight")
    plt.close()
    print("Saved tokenizer comparison plot to assets/tokenizer_comparison.png")

    print("\n--- Summary of Tokenizer Benchmarking Results ---")
    for r in results:
