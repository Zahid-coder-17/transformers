...
# 2. Performance Evaluation over Train & Validation Sets
@torch.no_grad()
...
print(f"Train Perplexity: {results['train_ppl']:.2f}")
...
# 3. Measure Generation Latency & Throughput
...
print(f"Generated {num_tokens_gen} tokens in {elapsed:.2f} seconds")
print(f"Throughput       : {throughput:.2f} tokens/sec")
print(f"Latency          : {ms_per_token:.2f} ms/token")

# 4. Generate Visual Graphics & Dashboard
...
# Subplot 1: Train vs Validation Loss Distribution
...
# Subplot 2: Character Prediction Accuracy Breakdown
...
# Subplot 3: Perplexity Comparison
...
# Subplot 4: Model Specifications & Benchmark Metrics Card
...
# 5. LLM Text Generation Samples Demonstration
print("\n" + "="*70)
print("             LLM TEXT GENERATION SAMPLES (Trained Model)")
print("="*70)
...
for prompt, temp, top_k, top_p in test_prompts:
    ...
    print(f"\n[Prompt: '{prompt}'] (Temp={temp}, Top-K={top_k}, Top-P={top_p})")
    print("-" * 65)
    print(story)
    print("-" * 65)

print("\nEvaluation and generation complete!")
